...
let createSeedMaps(groups: string array list) =
    groups |> List.removeAt 0 |> List.map (fun mapLines ->
        let source, destination = mapLines |> Array.item 0 |> fun line -> getGroups(line, @"(\w+)-\w+-(\w+)") |> getSourceAndDestination
        let ranges = mapLines |> Array.removeAt 0 |> Array.map (fun line ->
            let rangeNumbers = getMatches(line, @"\d+") |> Seq.map Int64.Parse |> Seq.toArray
...
let part1(lines: string array) =
    ...
    let seeds = groups |> List.item 0 |> Array.item 0 |> fun line -> getMatches(line, @"\d+") |> Seq.map Int64.Parse |> Seq.toArray
...
let part2(lines: string array) =
    let groups = lines |> getLinesGroupedByNewLine
    let seeds = groups |> List.item 0 |> Array.item 0 |> fun line -> getMatches(line, @"\d+") |> Seq.map Int64.Parse |> Seq.toArray |> getSeedRanges
    let seedMaps = createSeedMaps groups
