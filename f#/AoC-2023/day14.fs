...
let switchToColumns (cave: Cave array array) =
    ...
    cave
    |> Array.collect (fun caveRow ->
        y <- y + 1
        caveRow |> Array.mapi (fun x rock -> Point(x, y), rock))
...
let applyGravity (caveColumn: Cave array) =
    ...
    caveColumn
    |> Array.mapi (fun i cave ->
        if stonesToFall.ContainsKey(i) then
            stonesToFall[i]
        else (*if stonesToFall.Values |> Seq.exists (fun (_, cve) -> match cvs with | Stone n | Empty n | Ledge n -> n = i; | _ -> false))*)
            0, cave)
