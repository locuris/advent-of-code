    ...
    if numbers |> List.head |> Array.forall (fun n -> n = 0) then
        ...
        |> List.append
            [ numbers
              |> List.head
              |> Array.pairwise
              |> Array.map (fun (first, second) -> second - first) ]
        |> extrapolate
...
let getAnswer (histories: history array) : int =
    histories
    |> Array.sumBy (fun histories -> histories |> List.sumBy (fun history -> history |> Array.last))


let part1 (lines: string array) : string =
    lines |> getHistories |> getAnswer |> string
...
let part2 (lines: string array) : string =
    ...
    histories |> changeDirections |> Array.map extrapolate |> getAnswer |> string
