...
type InstructionSet =
    struct
        ...
        new(instructionSet: int array) = { instructions = instructionSet }
...
let rec traverseTree
    (tree: Map<string, string list>)
    (steps: int64)
    (instructionSet: InstructionSet)
    (endCondition: string -> bool)
    (node: string)
    : int64 =
    if endCondition (node) then
        steps
    else
        traverseTree tree (steps + 1L) instructionSet endCondition (tree[node][instructionSet.nextInstruction (steps)])

let rec traverseTrees
    (tree: Map<string, string list>)
    (nodes: string array)
    (step: int64)
    (instructionSet: InstructionSet)
    : int64 =
    if nodes |> Array.forall (_.EndsWith('Z')) then
        step
    else
        traverseTrees
            tree
            (nodes
             |> Array.map (fun node -> tree[node][instructionSet.nextInstruction (step)]))
            (step + 1L)
            instructionSet


let getInstructionsAndNodes (lines: string array) : InstructionSet * Map<string, string list> =
    let instructionInput, nodesInput =
        lines |> GetLinesGroupedByNewLine |> (fun groups -> groups[0][0], groups[1])

    let instructions = instructionInput |> InstructionSet

    let nodes =
        nodesInput
        |> Array.map (GetMatchesAsStringArray @"\w\w\w")
        |> Array.map (fun node -> (node[0], [ node[1]; node[2] ]))
        |> Map.ofArray

    instructions, nodes

let part1EndCondition (node: string) : bool = node = "AAA"
...
let part2EndCondition (node: string) : bool = node.EndsWith('Z')
...
let part2 (lines: string array) : string =
    ...
    let pathLengths =
        ...
        |> List.ofSeq
        |> List.map (traverseTree nodes 0L instructions part2EndCondition)
        |> List.sort
