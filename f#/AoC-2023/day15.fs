...
let createBox id = Box(id, Map.empty)

let InitialBoxes: Box array = [| 0..255 |] |> Array.map createBox

let hashAlgorithm (code: int) (chr: char) : int = (((chr |> int) + code) * 17) % 256

let hash (code: string) : BoxId =
    let mutable hashCode = 0
    code.ToCharArray() |> Array.iter (fun c -> hashCode <- hashAlgorithm hashCode c)
    hashCode
...
let createLens (input: string) : Lens =
    ...
    let (label, focalLength) =
        input.Split(LanguagePrimitives.EnumToValue operation)
        |> (fun split -> (split[0], (if split[1] = "" then 0 else int split[1])))
...
let processLens (boxes: Box array) (lens: Lens) : Box array =
    ...
    boxes
    |> Array.map (fun (id, map) -> if id = boxId then newBox else Box(id, map))
...
let part2 (lines: string array) : string =
    ...
    boxes
    |> Array.filter (fun (_, content) -> not content.IsEmpty)
    |> Array.sumBy (fun box ->
        let (boxId, contents) = box
        let sum = contents.Values |> Seq.sumBy (fun (pos, (_, f)) -> (1 + boxId) * pos * f)
        printfn $"{boxId} = {sum}"
        sum)
    |> string
