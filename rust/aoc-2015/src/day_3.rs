...
impl Point {
    ...
    pub fn next(&self, step: char) -> Point {
        match step {
...
fn calculate_houses(path: Chars<'_>) -> i32 {
    let mut set = HashSet::new();
    let mut last_point = Point {x: 0, y: 0};
    set.insert(last_point.clone());
    for step in path {
        last_point = last_point.next(step);
        set.insert(last_point);
    }
    set.len() as i32
}

fn calculate_houses_with_robot(path: Chars<'_>) -> i32 {
    let mut set = HashSet::new();
    let mut santa_point = Point {x: 0, y: 0};
    let mut robo_point = Point {x: 0, y: 0};
    let mut santa = true;
    set.insert(santa_point.clone());
    for step in path {
        if santa {
            santa_point = santa_point.next(step);
            set.insert(santa_point);
        } else {
            robo_point = robo_point.next(step);
            set.insert(robo_point);
        }
        santa = !santa;
    }
    set.len() as i32
...
mod test {
    ...
    #[test]
    fn test_part_2() {
        assert_eq!(3, calculate_houses_with_robot(String::from("^v").chars()));
        assert_eq!(3, calculate_houses_with_robot(String::from("^>v<").chars()));
        assert_eq!(11, calculate_houses_with_robot(String::from("^v^v^v^v^v").chars()));
    }

    #[test]
    fn input_part_2() {
        let answer = calculate_houses_with_robot(String::from(INPUT).chars());
        println!("Answer to part 2 is {answer}")
    }

    #[test]
    fn test_part_1() {
        assert_eq!(2, calculate_houses(String::from(">").chars()));
        assert_eq!(4, calculate_houses(String::from("^>v<").chars()));
        assert_eq!(2, calculate_houses(String::from("^v^v^v^v^v").chars()));
    }

    #[test]
    fn input_part_1() {
        let answer = calculate_houses(String::from(INPUT).chars());
